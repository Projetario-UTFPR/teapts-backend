            ...
            te.bindW("prismaActivity", ({ payload, snapshot }) =>
                te.tryCatch(
                    () => this.prisma.activity.create({
                        data: {
                            ...payload, activityReferringToDocuments: {
                    ...
                    (error) => new IrrecoverableError({
                        message: `Error occurred in ${PrismaActivityRepository.name} when creating the Activity '${JSON.stringify(payload)}'.`,
                        cause: error as Error,
                    })
                )
            ),
            te.map(({ prismaActivity }) => activityMapper.fromPrisma({
                ...prismaActivity,
                documents: prismaActivity.activityReferringToDocuments.map(relation => ({ id: relation.documentId })),
            }))
        )();
    }
}