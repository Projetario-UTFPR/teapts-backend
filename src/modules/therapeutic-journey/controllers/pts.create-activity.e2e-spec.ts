...
describe("[e2e] Create Activity - PTS Controller (v1)", () => {
    ...
    describe("POST /v1/pts/activity/create", () => {
        ...
        it("should return 404 if professional does not exist", { tags: ["createActivity"] }, async () => {
            const activeTimeline = ptsFactory.createTimeline({
                status: PtsTimeline.Status.Running,
            });
            ...
            expect(response.body).toHaveProperty("message");
            expect(response.body.message).toBe(`Não foi possível encontrar nenhum perfil profissional com id "${otherProfessionalId}".`);
        });

        it("should return 400 if document does not belong to patient", { tags: ["createActivity"] }, async () => {
            const activeTimeline = ptsFactory.createTimeline({
                status: PtsTimeline.Status.Running,
            });
            ...
            expect(response.body).toHaveProperty("message");
            expect(response.body.message).toBe(`O documento identificado por "${mockDocument.getId()}" não pertence ao paciente provido, ` +
                "e portanto não pode ser vinculado.",);
        });

        it("should return 403 if professional does not belong to multidisicplinary team", { tags: ["createActivity"] }, async () => {
            const activeTimeline = ptsFactory.createTimeline({
                status: PtsTimeline.Status.Running,
            });

            const otherAccount = await accountsFactory.createAndPersist(prisma);
            const otherProfessional = await professionalsFactory.createAndPersist(prisma, { account: otherAccount });
            ...
            expect(response.body).toHaveProperty("message");
            expect(response.body.message).toBe(`Profissional não autorizado a acessar o projeto terapêutico singular.`);
        });
        ...
        it(
            ...
            async () => {
                ...
                expect(response.body).toHaveProperty("message");
                expect(response.body.message).toBe(`O documento especificado não foi encontrado. ID: ${nonExistingDocumentId}`);
            },
        );
