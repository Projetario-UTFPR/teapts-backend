...
const ENDPOINT = (pts: ProjetoTerapeuticoSingular) => `/v1/pts/${pts.getId().toString()}/approve`;

describe("[e2e] PTS Controller :: Approve PTS (v1)", () => {
  let app: INestApplication<App>;
  let prisma: PrismaService;
  let tokensService: AssignTokenService;
  let underlyingService: ApproveDraftPtsService;
  ...
  beforeAll(async () => {
    ...
    underlyingService = app.get(ApproveDraftPtsService);
  ...
  it("should approve a given PTS for the authenticated patient", async () => {
    ...
    expect(ptsFromDb.status, "it should have been set to 'planning' status").toBe(
      PtsStatus.Planning,
    );
